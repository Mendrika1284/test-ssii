...
// });
events_1.EventEmitter.defaultMaxListeners = 40;
// * Config
// Gateway
const configGatewayOrigin = ["http://localhost:8080", "http://localhost:8082"];
const configGatewayDns = {
    url: "http://localhost:8080",
    trustProxy: false,
};
const configGatewayRateLimit = {
    limit: 400,
};
const configGatewayCookie = {
    path: "/",
    secure: true,
    httpOnly: true,
    sameSite: true,
};
const configGatewayToken = {
    audience: "edmp",
...
const configAppAdminUsers = [{ username: "admin", scope: "admin" }];
const configAppVersions = {
...
const moleculerConfig = {
    ...
    ServiceFactory: class ServiceFactory extends moleculer_1.Service {
        constructor(broker, schema) {
            super(broker, schema);
        }
    },
    // Custom Context class. If not null, broker will use it when creating contexts for requests & events. Default: null
    ContextFactory: class ContextFactory extends moleculer_1.Context {
        constructor(broker, endpoint) {
            var _a, _b;
            super(broker, endpoint);
            if ((_a = endpoint) === null || _a === void 0 ? void 0 : _a.action) {
                this.name = endpoint.action.name;
            }
            else if ((_b = endpoint) === null || _b === void 0 ? void 0 : _b.event) {
                this.name = endpoint.event.name;
            }
        }
    },
};
